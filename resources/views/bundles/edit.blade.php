...
@section('content')
<div class="max-w-2xl"
     x-data="bundleForm({{ json_encode($bundle->items->map(fn($i) => ['product_id' => (string)$i->product_id, 'qty' => (float)$i->qty])->values()) }})">
            ...
            <x-card title="Informasi Bundle">
                <div class="space-y-4">
                    ...
                    <div>
                        ...
                        <input type="text" name="description" value="{{ old('description', $bundle->description) }}"
                               class="w-full px-3 py-2.5 border border-gray-300 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-emerald-500"
                    ...
                    <div>
                        ...
                        <div class="relative">
                            ...
                            <input type="number" name="price" value="{{ old('price', $bundle->price) }}" min="0" step="500" required
...
@push('scripts')
<script>
function bundleForm(existingItems) {
    return {
        items: existingItems.length ? existingItems.map(i => ({product_id: String(i.product_id), qty: i.qty}))
                                     : [{ product_id: '', qty: 1 }],
