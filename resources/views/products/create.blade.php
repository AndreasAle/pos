    ...
    <x-card title="Data Produk Baru">
        <form method="POST" action="{{ route('products.store') }}" enctype="multipart/form-data" class="space-y-5">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div class="sm:col-span-2">
                    ...
                    <input type="text" name="name" value="{{ old('name') }}" required
                           ...
                           placeholder="Contoh: Es Kopi Susu, Nasi Ayam Geprek...">
                </div>
                <div>
                ...
                <div>
                    ...
                    <div class="relative">
                        <span class="absolute left-3 top-1/2 -translate-y-1/2 text-sm text-gray-500">Rp</span>
                        <input type="number" name="cost_price" value="{{ old('cost_price', 0) }}" min="0" step="500"
                               class="pl-10 w-full px-3 py-2.5 border border-gray-300 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-emerald-500">
                    </div>
                </div>
                @if($outlets->count() > 1)
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1">Outlet <span class="text-gray-400">(kosong = semua outlet)</span></label>
            ...
            <div class="flex items-center gap-3 pt-2 border-t border-gray-100">
                ...
                <a href="{{ route('products.index') }}" class="text-sm text-gray-500 hover:text-gray-700">Batal</a>
            </div>
        </form>
    </x-card>
</div>
